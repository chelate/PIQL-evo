...
function build_pickle_parameters(size, holes, thick, basevalue,reward_pos)
    pickle = convert.(Int64, ones(Int64, size) .* basevalue)
    pickle[1, :] .= 0
    pickle[size[1], :] .= 0
    pickle[:, 1] .= 0
    ...
    for hole = 1:holes
        point = start_random_actor(pickle, basevalue)
        if !(reward_pos==give_cartisian_posistion(pickle, point))
            push!(walls,give_cartisian_posistion(pickle, point))
            pickle[point] = 0
        end
